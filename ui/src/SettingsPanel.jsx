...
const SettingsPanel = ({ isMobile }) => {
    ...
    const fetchIntegrations = async () => {
        try {
            ...
            if (data['gemini']) setGeminiKey(data['gemini'].api_key);
        } catch (err) {
                ...
                >
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-bg-surface border border-border rounded-[24px] overflow-hidden premium-border group transition-all hover:border-accent/10 shadow-sm">
                            <div className={`${isMobile ? 'p-6' : 'p-8'} space-y-8`}>
                                <div className="flex items-center gap-5">
                                    <div className="w-14 h-14 rounded-2xl overflow-hidden border border-border flex items-center justify-center bg-bg-hover group-hover:bg-[#4285F4]/5 transition-all">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <label className="block text-[10px] font-black uppercase tracking-widest text-text-muted/40 ml-1">Google AI API Key</label>
                                        <input
                                            type="password"
                                            value={geminiKey}
                                            onChange={(e) => setGeminiKey(e.target.value)}
                                            placeholder="AIza..."
                                            className="w-full bg-bg-hover border border-border rounded-xl px-5 py-4 text-sm text-text-main placeholder-text-muted/20 focus:outline-none focus:border-[#4285F4]/40 transition-all font-mono"
                                        />
                                        <p className="text-[10px] text-text-muted/40 ml-1">Get your key at <span className="text-[#4285F4]">aistudio.google.com</span></p>
                                    </div>
                                    <button
                                        onClick={() => saveIntegration('gemini', geminiKey)}
                                        disabled={saving}
                                        className="w-full font-black py-4 rounded-xl active:scale-[0.98] transition-all disabled:opacity-20 text-[10px] uppercase tracking-[0.2em] shadow-md"
                                        style={{ background: '#4285F4', color: '#fff' }}
                                    >
                                        {saving ? 'Syncing...' : 'Save Gemini Key'}
                                    </button>
                                </div>
                            </div>
                        </div>
